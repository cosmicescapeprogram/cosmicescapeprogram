...
方法：為損失函數加上權重的平方范數（L2范數） $$\frac{1}{2} \lambda W^2$$  →→在求權重梯度的計算中，要為之前的誤差反向傳播法的結果加上正則化項的導數 $$\lambda W$$   

+ $$\lambda$$ 是控制正則化強度的超參數（設置越大，對大的權重施加的懲罰就越重）  
+ 1/2是能夠使范數的求導結果變為 $$\lambda W$$ 的調整用常量
